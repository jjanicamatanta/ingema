<div class="module-small bg-dark">
  <div class="container">
    <div class="row">
      <div class="col-sm-3">
        <div class="widget">
          <h5 class="widget-title font-alt">Contacto</h5>
          <p>Dirección: Jr. Acuarinas N° 253, Los Olivos - Lima</p>
          <p>Email: contacto@ingema.com</p>
          <p>Tlf: 983427545</p>
        </div>
      </div>
      <div class="col-sm-3">
        <div class="widget">
          <h5 class="widget-title font-alt">Conócenos</h5>
          <ul class="icon-list">
            <li><a href="/">Ingema</a></li>
            <li><a href="//#about">Acerca de nosotros</a></li>
            <li><a href="//#mejor_opcion">La mejor opción</a></li>
            <li><a href="/#team">Nuestro Equipo</a></li>
            <li><a href="/#clients">Nuestros Clientes</a></li>
          </ul>
        </div>
      </div>
      <div class="col-sm-3">
        <div class="widget">
          <h5 class="widget-title font-alt">Servicios</h5>
          <ul class="icon-list">
            <li><a href="/services.php#demo-card--step1">Estudios Ambientales</a></li>
            <li><a href="/services.php#demo-card--step2">Asesoramiento en Materia Jurídica</a></li>
            <li><a href="/services.php#demo-card--step3">Asesoramiento en Materia de emisiones a la atmosfera</a></li>
            <li><a href="/services.php#demo-card--step4">Asesoramiento en Materia de Vertidos</a></li>
            <li><a href="/services.php#demo-card--step5">Asesoramiento Ambiental en RR.SS.</a></li>
            <li><a href="/services.php#demo-card--step6">Asesoramiento Ambiental en Materia de Envases</a></li>
            <li><a href="/services.php#demo-card--step7">Asesoramiento sobre Manejro de RR.SS</a></li>
          </ul>
        </div>
      </div>
      <div class="col-sm-3">
        <div class="widget">
          <h5 class="widget-title font-alt">Nuestro Equipo</h5>
          <ul class="widget-posts">
            <li class="clearfix">
              <div class="widget-posts-image"><a href="#"><img src="assets/images/rp-1.jpg" alt="Post Thumbnail"></a></div>
              <div class="widget-posts-body">
                <div class="widget-posts-title"><a href="#">J. Raúl Anicama B.</a></div>
                <div class="widget-posts-meta">Director</div>
              </div>
            </li>
            <li class="clearfix">
              <div class="widget-posts-image"><a href="#"><img src="assets/images/rp-2.jpg" alt="Post Thumbnail"></a></div>
              <div class="widget-posts-body">
                <div class="widget-posts-title"><a href="#">J. Carlos Anicama T.</a></div>
                <div class="widget-posts-meta">Gerente General</div>
              </div>
            </li>
            <li class="clearfix">
              <div class="widget-posts-image"><a href="#"><img src="assets/images/rp-2.jpg" alt="Post Thumbnail"></a></div>
              <div class="widget-posts-body">
                <div class="widget-posts-title"><a href="#">J. Amparo Tanta T.</a></div>
                <div class="widget-posts-meta">Jefe de operaciones</div>
              </div>
            </li>
            <li class="clearfix">
              <div class="widget-posts-image"><a href="#"><img src="assets/images/rp-2.jpg" alt="Post Thumbnail"></a></div>
              <div class="widget-posts-body">
                <div class="widget-posts-title"><a href="#">J. ALberto Anicama T.</a></div>
                <div class="widget-posts-meta">Auxiliar de servicios</div>
              </div>
            </li>
          </ul>
        </div>
      </div>
    </div>
  </div>
</div>
<hr class="divider-d">
<footer class="footer bg-dark">
  <div class="container">
    <div class="row">
      <div class="col-sm-9">
        <p class="copyright font-alt">&copy; 2018&nbsp;<a href="/">J.A. Ingenieria y Medio Ambiente S.A.C. </a>, Todos los derechos reservados</p>
      </div>
      <div class="col-sm-3">
        <div class="footer-social-links"><a href="#"><i class="fa fa-facebook"></i></a><a href="#"><i class="fa fa-twitter"></i></a><a href="#"><i class="fa fa-dribbble"></i></a><a href="#"><i class="fa fa-skype"></i></a>
        </div>
      </div>
    </div>
  </div>
</footer>
